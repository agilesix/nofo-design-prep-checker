...
- **Links** — internal and external link issues, including non-descriptive link text, basic formatting problems, and broken internal anchors (with fuzzy-match suggestions when a likely target can be found; suggested anchors are derived from heading text, with spaces converted to underscores — e.g. "Attachment 1: Overview" → `Attachment_1_Overview`)
