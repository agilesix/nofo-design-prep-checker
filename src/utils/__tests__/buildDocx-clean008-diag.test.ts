...
describe('CLEAN-008 diagnostic — realistic OOXML scenarios', () => {
  ...
  it('DIAG-1: updates w:anchor in a full-namespace OOXML document (no prior serialization cycle)', async () => {
    ...
    console.log('[DIAG-1] Output XML snippet (hyperlink area):');
    const hyperlinkIdx = outXml.indexOf('w:hyperlink');
    console.log('[DIAG-1]', outXml.slice(Math.max(0, hyperlinkIdx - 20), hyperlinkIdx + 200));

    const anchor = extractHyperlinkAnchor(outXml);
    console.log('[DIAG-1] Extracted hyperlink anchor:', anchor);
    expect(anchor).toBe('Contacts_and_Support');

    const headingText = extractParagraphText(outXml, 0);
    console.log('[DIAG-1] Extracted heading text:', headingText);
  ...
  it('DIAG-2: updates w:anchor AFTER a prior serialize cycle caused by CLEAN-004 (double-space fix)', async () => {
    ...
    console.log('[DIAG-2] Output XML snippet (hyperlink area):');
    const hyperlinkIdx = outXml.indexOf('hyperlink');
    console.log('[DIAG-2]', outXml.slice(Math.max(0, hyperlinkIdx - 50), hyperlinkIdx + 300));

    const anchor = extractHyperlinkAnchor(outXml);
    console.log('[DIAG-2] Extracted hyperlink anchor:', anchor);
    expect(anchor).toBe('Contacts_and_Support');

    const headingText = extractParagraphText(outXml, 0);
    console.log('[DIAG-2] Extracted heading text:', headingText);
    expect(headingText).toBe('Contacts and Support');
