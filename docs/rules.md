...
|---------|-------|----------|-------------|
| HEAD-001 | Heading capitalization | Suggestion | H2 headings must use title case (all major words capitalized); H3–H6 headings must use sentence case (only first word and proper nouns capitalized). The rule flags H2 headings that appear to use sentence case and H3–H6 headings that appear to use title case. Detection is heuristic: ALL-CAPS acronyms (e.g. CDC, HRSA), minor words (articles, short prepositions, conjunctions), and the first word of the heading and any word immediately after a colon are excluded from the check. Because proper nouns and formal names (e.g. organization names, program names, form names like SF-424) cannot be detected reliably, issues are surfaced as suggestions only — the user must confirm whether the capitalization is correct. No auto-fix; corrections must be made in the source Word document. |
...
| LINK-007 | [PDF] label on external PDF links (auto-apply) | — | External hyperlinks whose URL contains ".pdf" (case-insensitive) must include "[PDF]" at the end of the link text. Three cases: (1) link text does not contain "[PDF]" anywhere — " [PDF]" is appended silently; (2) link text already ends with "[PDF]" (case-insensitive) — no change; (3) "[PDF]" appears as plain text immediately after the hyperlink but is not part of the link text — "[PDF]" is moved inside the link text and the adjacent plain-text occurrence is removed. Excludes internal bookmark links (`#…`) and `mailto:` links. No entry appears in the auto-applied list when no qualifying links are found. |
