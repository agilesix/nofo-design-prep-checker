...
| LINK-003 | Grants.gov capitalization (auto-apply) | — | Corrects "grants.gov" capitalization to "Grants.gov" everywhere it appears in the document — both in hyperlink text and in plain body text. The fix is applied case-insensitively (catches "grants.gov", "GRANTS.GOV", "Grants.Gov", etc.) and replaces only the matched substring; surrounding text, run properties, and hyperlink URLs are left untouched. A summary entry ("Grants.gov capitalization corrected in N location(s).") is added to the auto-applied changes when any corrections are made. |
