...
| CLEAN-018 | Remove instruction box tables (auto-apply) | — | Scans `word/document.xml` for single-cell tables (exactly one `w:tc`) whose first paragraph text contains the word "instructions" (case-insensitive). Qualifying tables are silently removed from the downloaded document. This covers all template instruction-box patterns, including `DGHT-SPECIFIC INSTRUCTIONS`, `DGHP-SPECIFIC INSTRUCTIONS`, `[WORD]-SPECIFIC INSTRUCTIONS` variants, and any single-cell table whose opener includes the word "instructions". Detection is OOXML-based and applies to all content guides regardless of which content guide is selected. No entry appears in the auto-applied list when no qualifying tables are found. |
