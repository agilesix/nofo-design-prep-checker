...
| LINK-006 | Internal link may not work in NOFO Builder | Warning | An internal anchor link (`#bookmark`) points to a target that could not be verified in the document. All non-exact-match cases surface an instruction-only warning: *"This internal link may be broken. To fix it, select the link text in Word, go to Insert → Link → This Document, and select the correct heading. Do not edit the link URL directly."* The tool does not rewrite `w:anchor` values; NOFO Builder requires links created through Word's native heading-link UI. Fuzzy matching (normalize-and-compare, numeric-suffix stripping, numeric extraction) is retained solely to identify the probable target heading so a **link-text suggestion** can be emitted when the link text does not already name the destination heading. |
