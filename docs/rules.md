...
| HEAD-004 | Heading may be too long | Suggestion | Per WCAG 2.0 G130, headings should be descriptive and concise. Flags H3–H6 headings that exceed 10 words or 80 characters (whichever threshold is reached first). H1 (NOFO title) and H2 (step titles) are excluded. Headings that appear to be entirely a proper noun phrase — every significant word starts with an uppercase letter — are also excluded, as long organization names are not heading length violations. **Suppression:** HEAD-004 is not emitted for any heading that exceeds HEAD-005's thresholds (>20 words or >150 characters) — see HEAD-005. **Accept-to-fix:** a text input pre-filled with the current heading text is shown; the user can enter a shorter replacement and accept to apply it to the downloaded document. The heading level (H3–H6) is preserved exactly — only the `w:t` text content is updated, never the `w:pStyle` or run formatting. If the user accepts without changing the prefilled text, no change is applied. Applies to all content guides. |
| HEAD-005 | Heading may be misformatted normal text | Suggestion | Flags H3–H6 headings that exceed 20 words or 150 characters — lengths that are strongly indicative of body text accidentally styled as a heading. H1 and H2 are excluded. Headings ending with a colon are also excluded regardless of length, as these are intentional section labels. **Suppression:** when HEAD-005 fires on a heading, HEAD-004 is not emitted for that same heading. Additionally, any heading that exceeds HEAD-005's thresholds (>20 words or >150 characters) is excluded from HEAD-004 even if HEAD-005 does not fire (e.g. colon-ending headings). **Accept-to-fix:** an accept/skip card with no text input is shown. Accepting changes the paragraph style (`w:pStyle`) from the current Heading style to `Normal` in the downloaded document. Text content and all run-level formatting (font, size, bold) are left unchanged — only the paragraph style changes. Applies to all content guides. |
...
| LINK-003 | Grants.gov capitalization (auto-apply) | — | Corrects "grants.gov" capitalization to "Grants.gov" across all story parts of the document — body text, footnotes, endnotes, headers, and footers — in both hyperlink text and plain text runs. The fix is applied case-insensitively ("grants.gov", "GRANTS.GOV", "Grants.Gov", etc.) and replaces only the matched substring within each text node; surrounding text, run properties, and hyperlink URLs are left untouched. Occurrences of "grants.gov" split across multiple adjacent text runs (uncommon, but possible when Word applies mixed inline formatting mid-word) are not corrected. A summary entry ("Grants.gov capitalization corrected in N location(s).") is added to the auto-applied changes when any corrections are made. |
...
| Rule ID | Title | Severity | Description |
|---------|-------|----------|-------------|
| NOTE-001 | Footnotes detected | Warning | Emitted when the document contains live footnotes. Detection scans word/document.xml for w:footnoteReference elements that are not inside a w:del or w:moveFrom ancestor, then cross-references against word/footnotes.xml to confirm the referenced IDs exist as user-authored entries. The warning message includes the count of live footnotes and instructs the user to convert them to endnotes in Word (References → Convert to Endnotes) before importing into NOFO Builder. Dismissible only — no auto-fix is applied. |
...
| CLEAN-010 | Normalize list item punctuation (auto-apply) | — | For each bulleted or numbered list with 3 or more items where at least one item already ends with a period, adds a trailing period to list items that do not already end with terminal punctuation (`.`, `?`, `!`, `:`, `;`, `,`). Only a period (`.`) acts as the trigger — lists where no items end with a period are left unchanged. Empty list items are skipped. Lists with fewer than 3 items are left unchanged. No entry appears in the auto-applied list when no qualifying lists are found. |
...
| CLEAN-017 | Correct "grants.gov" capitalization (auto-apply) | — | Replaces all case-insensitive occurrences of "grants.gov" with "Grants.gov" in every `w:t` text run in `word/document.xml`, including runs inside `w:hyperlink` elements. Only text content is updated — URLs, hyperlink relationships, and all run-level formatting are untouched. Detection parses the document HTML body text for occurrences of "grants.gov" (any case) that are not already the correct "Grants.gov"; if any are found, the fix is applied on download. Summary entry: "Grants.gov capitalization corrected in N location(s)." No entry appears in the auto-applied list when no corrections are needed. Applies to all content guides. |
| CLEAN-018 | Remove instruction box tables (auto-apply) | — | Scans `word/document.xml` for single-cell tables (exactly one `w:tc`) whose first paragraph text contains the word "instructions" (case-insensitive). Qualifying tables are silently removed from the downloaded document. This covers template instruction-box patterns such as `[WORD]-SPECIFIC INSTRUCTIONS` variants and other single-cell tables whose opener includes the word "instructions", except for the `BCD6F4`-shaded CDC instruction boxes. In particular, `DGHT-SPECIFIC INSTRUCTIONS` and `DGHP-SPECIFIC INSTRUCTIONS` boxes are covered here only when they are not the excluded `BCD6F4`-shaded variant; the shaded `DGHT`/`DGHP` instruction boxes are handled by `CLEAN-007` under CDC guides. Detection is OOXML-based and applies to all content guides regardless of which content guide is selected, subject to that exclusion. No entry appears in the auto-applied list when no qualifying tables are found. |
