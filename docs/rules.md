...
| CLEAN-017 | Rule shipped by the checker | — | This rule is implemented and registered in `src/rules/index.ts` and is included here so the universal CLEAN rules table stays aligned with the rules shipped by the checker. |
